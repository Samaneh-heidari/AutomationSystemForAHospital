...
{
    /// <summary>
    /// Represents the FarsiDate calendar.
    /// </summary>
    /// <version>1.0.2.0</version>
    [Serializable]
    public class FarsiDate : Calendar
    {
        /// <summary>
        ...
        public override System.DateTime ToDateTime(int year, int month, int day, int hour, int minute, int second, int millisecond, int era)
        {
            ...
            System.DateTime timePart = new System.DateTime(1, 1, 1, hour, minute, second, millisecond);
            long ticks = days * 864000000000L + timePart.Ticks + 195721056000000000L;
            System.DateTime dateTime;
            try
            {
                dateTime = new System.DateTime(ticks);
            }
            catch (System.ArgumentOutOfRangeException)
            {
        ...
        private void CheckTicksRange(bool validate, System.DateTime time)
        {
        ...
        private void GetYearAndRemainingDays(bool validate, System.DateTime time, out int year, out int days)
        {
            CheckTicksRange(validate, time);
            days = (time - new System.DateTime(196036416000000000L)).Days;
            year = 1;
        ...
        public string GetPersianDate(System.DateTime time, bool WithSlash)
        {
            string strDate;

            if (WithSlash)
                strDate = GetYear(time).ToString() + "/" + ((GetMonth(time) < 10) ? "0" : "") + GetMonth(time).ToString() + "/" + ((GetDayOfMonth(time) < 10) ? "0" : "") + GetDayOfMonth(time).ToString();
            else
                strDate = GetYear(time).ToString() + ((GetMonth(time) < 10) ? "0" : "") + GetMonth(time).ToString() + ((GetDayOfMonth(time) < 10) ? "0" : "") + GetDayOfMonth(time).ToString();
